...
class WebViewStore {
    ...
    public actionSheetElement: React.MutableRefObject<any> = React.createRef(); // 需要使用React.MutableRefObject<T>
...
@observer export class WebViewPage extends Component<IWebViewProps> {
    ...
    static getPageScreenOptions = ({ route, navigation }: NavigationProps) => {
    ...
    private updateTitle(title: string) {
        ...
        this.props.navigation.setParams({ title }); // 设置navigation title
    }
        ...
        return [
            {
                ...
                handle: () => {
                    Linking.openURL(webViewStore.url).catch((error: any) => console.warn('Open url failed:', error));
                }
            },
            {
                title: i18n.t(LANGUAGE_KEYS.COPY_URL),
                handle: () => {
                    Clipboard.setString(webViewStore.url);
                }
            }
        ];
    }
